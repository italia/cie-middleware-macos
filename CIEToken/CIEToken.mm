...
{
    LOG_INFO("[CTK] initWithSmartCard");
    LOG_INFO("[CTK] initWithSmartCard - smartCard slot name: %s", smartCard.slot.name.fileSystemRepresentation);
    ...
    if(!hModule)
    {
        ...
        {
            LOG_ERROR("[CTK] initWithSmartCard - Middleware functions list not found");
            dlclose(hModule);
            hModule = NULL;
            NSMutableDictionary *errorDetail = [NSMutableDictionary dictionary];
        ...
        if(rv != CKR_OK)
        {
            LOG_ERROR("[CTK] initWithSmartCard - Middleware functions list fails");
            dlclose(hModule);
            hModule = NULL;
            NSMutableDictionary *errorDetail = [NSMutableDictionary dictionary];
    ...
    if(!initPKCS11())
    {
        LOG_ERROR("[CTK] initWithSmartCard - Middleware's PKCS11 init fails");
        dlclose(hModule);
        hModule = NULL;
        NSMutableDictionary *errorDetail = [NSMutableDictionary dictionary];
    ...
    {
        LOG_ERROR("[CTK] initWithSmartCard - Middleware's getSlotList fails");
        if(pSlotList)
            free(pSlotList);
    ...
    for(i=0; i< ulCount; i++)
    {
        ...
        if(rv == CKR_OK && memcmp(smartCard.slot.name.fileSystemRepresentation, szDescription, sLen ) == 0){
             ...
             //LOG_DEBUG("[CTK] initWithSmartCard - pSlotList[%d]: %s", i, pSlotList[i]);
             if (rv != CKR_OK)
             {
                 LOG_ERROR("[CTK] initWithSmartCard - Middleware's GetTokenInfo(1) fails");
                 free(pSlotList);
                 closePKCS11();
                 dlclose(hModule);
                 hModule = NULL;
     ...
     if (i>= ulCount)
     {
        ...
        if (rv != CKR_OK)
        {
            LOG_ERROR("[CTK] initWithSmartCard - Middleware's GetTokenInfo(2) fails");
            free(pSlotList);
            closePKCS11();
            dlclose(hModule);
            hModule = NULL;
    ...
    LOG_INFO("[CTK] initWithSmartcard - token ID: %s", [instanceID UTF8String]);
    ...
    if(!findObject(_hSession, template_ck, 1, phObject, &ulObjCount) || ulObjCount == 0)
    {
        ...
        LOG_ERROR("[CTK] Middleware's findObject fails");
        NSMutableDictionary *errorDetail = [NSMutableDictionary dictionary];
    ...
    if (rv != CKR_OK)
    {
        ...
        LOG_ERROR("[CTK] Middleware C_GetAttributeValue fails");
        NSMutableDictionary *errorDetail = [NSMutableDictionary dictionary];
...
//
    ...
    {
//        error(rv);
        LOG_ERROR("[CTK] getSlotList - error: %d", rv);
        return NULL_PTR;
    }
...
//            error(rv);
        ...
        LOG_DEBUG("[CTK] getSlotList - bPresent: %d, bSlotList: %d", bPresent, *pSlotList);
        return pSlotList;
    }
    else
    {
...
//
    ...
    {
//        error(rv);
        LOG_ERROR("[CTK] openSession - cannot open session, err: %d", rv);
        return NULL_PTR;
    }
...
void closeSession(CK_SESSION_HANDLE hSession)
{
    ...
    {
        //        error(rv);
        LOG_ERROR("[CTK] closeSession - cannot close session, err: %d", rv);
        return;
    }
