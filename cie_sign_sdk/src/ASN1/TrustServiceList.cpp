...
bool CTrustServiceList::Initialize()
{
...
#else
    // macOS: usa lo stesso percorso sandbox dove vengono salvati log e cache
    ...
    {
        if(mkdir(sharedFolderPath.c_str(), 0700) != 0)
        {
            LOG_ERR((0, "CTrustServiceList::Initialize", "Failed to create CIEPKI directory"));
        }
    }
        
    snprintf(szCacheFile, MAX_PATH, "%s/tsl_cache.dat", sharedFolderPath.c_str());
    LOG_DBG((0, "CTrustServiceList::Initialize", "Cache file path: %s", szCacheFile));
#endif
