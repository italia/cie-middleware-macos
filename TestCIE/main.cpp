...
bool digest(CK_SESSION_HANDLE hSession, CK_MECHANISM_TYPE mechanism)
{
	...
	if (g_nLogLevel > 2)
		std::cout << "  -> Calcola l'hash: " << std::endl;
...
bool encryptDecrypt(CK_SESSION_HANDLE hSession)
{
	...
	dataVal.append((const BYTE*)szToEncrypt, (unsigned long)strlen(szToEncrypt));
...
int main(int argc, char* argv[])
{
	...
	if (!hModule)
	{
		std::cout << "  -> Modulo " << szCryptoki << " non trovato " << dlerror() << std::endl;
		exit(1);
	}
#endif
